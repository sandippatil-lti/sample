...
/**
 * class OrgAwareLDAPToPortalConverter: A custom LDAPToPortalConverter that
 * maps LDAP groups to Liferay Organizations.
 * @author dnebinger
 */
@Component(immediate = true, service = LDAPToPortalConverter.class)
public class OrgAwareLDAPToPortalConverter implements LDAPToPortalConverter {
	...
	public LDAPUser importLDAPUser(
		...
		// The group assignments from LDAP must be mapped over to Liferay
		// Organization memberships.

		// get the list of group memberships from the attributes

		String[] groups = LDAPUtil.getAttributeStringArray(
			attributes, userMappings, UserConverterKeys.GROUP);

		// if the user has one or more groups

		if (ArrayUtil.isNotEmpty(groups)) {

			// use a set to collect org ids
			...
			for (String group : groups) {
				...
				// try to find an existing org with the group name

				Organization organization = _organizationPersistence.fetchByC_N(
					companyId, group);

				if (Validator.isNotNull(organization)) {

					// add the org id to the set

					organizationIds.add(organization.getOrganizationId());
				}
			}

			// if at least one organization has been identified

			if (SetUtil.isNotEmpty(organizationIds)) {

				// set the array of org ids to the user

				ldapUser.setOrganizationIds(
	...
	@Reference(unbind = "-")
	protected void setOrganizationPersistence(
		OrganizationPersistence organizationPersistence) {

		_organizationPersistence = organizationPersistence;
	}
	...
	private static final Log _log = LogFactoryUtil.getLog(
		OrgAwareLDAPToPortalConverter.class);
